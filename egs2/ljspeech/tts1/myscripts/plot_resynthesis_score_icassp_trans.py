...
def plot(axes, df, num):
    ...
    if num == 0:
        ...
        ax.set_ylabel("(a) Tacotron2", fontsize=12)
    if num == 1:
        ax.set_ylabel("(b) VITS", fontsize=12)
    ax.set_ylim(-5, 95)
    ...
    ax.grid()
    if num == 0:
        ax.set_title(r"CER (<10)", fontsize=12)
    for N in [20, 40, 80, 120, 160, 200, 240, 280]:
        indices = [f"fixed_{N}-{2**i}_dedup" for i in range(7, 7 + 8)]
    ...
    ax.grid()
    if num == 0:
        ax.set_title(r"UTMOS$\uparrow$", fontsize=12)
    for N in [20, 40, 80, 120, 160, 200, 240, 280]:
        indices = [f"fixed_{N}-{2**i}_dedup" for i in range(7, 7 + 8)]
        ax.plot(
            list(df.loc[indices, "bitrate"]),
...
if __name__ == "__main__":
    ...
    fig.savefig("./fig/resynthesis_score_trans.pdf")
